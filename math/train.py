...
class TrainingArguments(transformers.TrainingArguments):
    ...
    init_weights: bool | str = field(
        default=True,
        metadata={
            "help": (
                "True -> LoRA; `pissa` -> PiSSA; `pissa_niter_16` -> Fast SVD PiSSA"
            ),
        },
    )
    ...
    # TuckA setting
    use_tucka: Optional[bool] = field(default=False)
    tucka_r: Optional[int] = field(default=32)
    tucka_k: Optional[int] = field(default=6)
    tucka_t: Optional[int] = field(default=2)
    tucka_p: Optional[int] = field(default=2)
    tucka_alpha: Optional[int] = field(default=1)
    tucka_ec_perturb_scale: Optional[float] = field(
        default=0.1,
        metadata={"help": "The perturbation scale for the expert centroids in TuckA."},
    )
    tucka_use_kaiming_init_ec: Optional[bool] = field(default=False)
    # HRA setting
    use_hra: Optional[bool] = field(default=False)
    hra_r: Optional[int] = field(default=32)
    # BOFT setting
    use_boft: Optional[bool] = field(default=False)
    boft_block_size: Optional[int] = field(default=16)
...
def get_last_checkpoint(checkpoint_dir):
    if os.path.isdir(checkpoint_dir):
        ...
        for filename in os.listdir(checkpoint_dir):
            ...
            ) and filename.startswith(PREFIX_CHECKPOINT_DIR):
                max_step = max(
                    max_step, int(filename.replace(PREFIX_CHECKPOINT_DIR + "-", ""))
                )
        if max_step == 0:
            return None
...
def build_model(script_args, checkpoint_dir):
    ...
    if not script_args.full_finetune:
        ...
        if checkpoint_dir is not None:
            ...
            logger.info(
                f"Initilize LoRA/PiSSA/CLOVER/TuckA adapters from {script_args.model_name_or_path}/{script_args.adapter_name_or_path}."
            )
        ...
        elif script_args.use_tucka:
            peft_config = TuckAConfig(
                r=script_args.tucka_r,
                k=script_args.tucka_k,
                t=script_args.tucka_t,
                p=script_args.tucka_p,
                task_type=TaskType.CAUSAL_LM,
                log_expert_load=200,
                ec_perturb_scale=script_args.tucka_ec_perturb_scale,
                alpha=script_args.tucka_alpha,
                use_kaiming_init_ec=script_args.tucka_use_kaiming_init_ec,
                target_modules=script_args.target_modules.split(","),
            )
            model = get_peft_model(model, peft_config)
        elif script_args.use_hra:
            ...
            peft_config = LoraConfig(
                ...
                runtime_config=LoraRuntimeConfig(
                    ephemeral_gpu_offload=script_args.use_dora
                ),
                task_type=TaskType.CAUSAL_LM,
                target_modules=script_args.target_modules.split(","),
...
def train():
    ...
    if script_args.local_rank == 0:
        logger.info(
            "Load tokenizer from {} over.".format(script_args.model_name_or_path)
        )
    ...
    elif not script_args.full_finetune and not script_args.merge:
        model.save_pretrained(script_args.output_dir)
        tokenizer.save_pretrained(script_args.output_dir)
    if script_args.full_finetune:
        safe_save_model_for_hf_trainer(
            trainer=trainer, output_dir=script_args.output_dir
        )
