    ...
    **Attributes**:
    ...
    def __init__(
        ...
        if self._is_prompt_learning:
            ...
            self.add_adapter(
                adapter_name, peft_config, low_cpu_mem_usage=low_cpu_mem_usage
            )
        else:
    ...
    def from_pretrained(
        ...
        **kwargs: Any,
        ...
        missing_keys = [
            ...
            if "vblora_vector_bank" not in k and "prompt_encoder" not in k
        ]
        if missing_keys:
            ...
            warnings.warn(
                f"Found missing adapter keys while loading the checkpoint: {missing_keys}"
            )
    ...
    def _setup_prompt_encoder(self, adapter_name: str):
        ...
        try:
            ...
            # Roberta, Deberta, etc.
            word_embeddings = self.base_model.get_submodule(
        ...
        if word_embeddings is None:
            ...
            for named_param, value in list(transformer_backbone.named_parameters()):
                ...
                if value.shape[0] == self.base_model.config.vocab_size or (
                    ...
                    word_embeddings = transformer_backbone.get_submodule(
        ...
        if config.peft_type == PeftType.PROMPT_TUNING:
            ...
            if any(
                ...
                for module in self.get_base_model().modules()
            ):
                raise ValueError(
            ...
            if (
                ...
                past_key_values.is_updated = {
                    ...
                    for layer_idx in range(
                        len(past_key_values.cross_attention_cache.key_cache)
                    )
                }
            map_cache_to_layer_device_map(
                self.get_base_model(), past_key_values
            )  # no-op if not a Cache instance
    ...
    def _check_new_adapter_config(
        self, peft_config: PeftConfig, is_trainable: bool
    ) -> None:
        ...
        if len(all_configs) > 1:
            if any(
                getattr(config, "init_lora_weights", None) == "pissa"
                ...
                getattr(config, "init_lora_weights", None) == "olora"
                for config in all_configs
            ):
                msg = (
...
class PeftModelForCausalLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            return self._cpt_forward(
                input_ids, inputs_embeds, peft_config, task_ids, batch_size, **kwargs
            )
        else:
            if inputs_embeds is None:
                inputs_embeds = self.word_embeddings(input_ids)
    ...
    def _cpt_forward(
        ...
        **kwargs,
        ...
        device = [
            i.device for i in [input_ids, inputs_embeds, labels] if i is not None
        ][0]
        ...
        if labels is not None:
            ...
            adjusted_input_type_mask[adjusted_input_type_mask > 0] += (
                prefix_type_mask.max()
            )
            # Concatenate prefix and shifted input type masks
            cpt_type_mask = torch.cat(
                (prefix_type_mask, adjusted_input_type_mask), dim=1
            )
