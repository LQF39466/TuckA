...
def get_peft_model_state_dict(
        ...
        if config.save_only_topk_weights:
            ...
            for k in state_dict:
                if "vblora_logits" in k:
                    ...
                    to_return.update(
                        {
                            k + "_topk_weights": torch.softmax(logits, dim=-1)[
                                :, :, :-1
                            ].contiguous()
                        }
                    )
    ...
    elif config.peft_type == PeftType.TUCKA:
        to_return = {k: state_dict[k] for k in state_dict if "tucka_" in k}
    else:
...
def set_peft_model_state_dict(
    ...
    if low_cpu_mem_usage:
        load_result = model.load_state_dict(
            peft_model_state_dict, strict=False, assign=True
        )
